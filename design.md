...
## App implementation (nm-attendance)

- **Canonical CSS tokens:** `tokens.css` (Geist palette + semantic aliases for existing components)
- **shadcn bridge:** `src/app/globals.css`
- **Typography utilities:** `src/app/geist-typography.css`
- **Fonts:** `geist` package — `GeistSans` + `GeistMono` in `src/app/layout.tsx`
- **Surface themes:** `src/app/theme-presets.css` (`default` = Geist light, `night` = dark)

Semantic aliases used in app code:

| Alias | Geist token |
|-------|-------------|
| `--color-paper` | `background-100` |
| `--color-paper-2` | `background-200` |
| `--color-paper-3` | `gray-100` |
| `--color-ink` | `gray-1000` |
| `--color-ink-2` | `gray-900` |
| `--color-ink-muted` | `gray-700` |
| `--color-accent` | `gray-1000` |
| `--color-link` / `--color-focus` | `blue-700` |
| `--color-destructive` | `red-800` |
